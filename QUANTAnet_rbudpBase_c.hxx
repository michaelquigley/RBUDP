 ...
 * RBUDP base class.
 *
 * This enables us to transfer a block of memory over UDP reliably.  This base class cannot be instantiated.
 *
 * TCP needs fine tune over long fat pipes.  But UDP can get the maximum performance.  With RBUDP, we can get both
 * maximum performance and reliable transfer. RBUDP try to keep the network pipe as full as possible during the bulk
 * data transfer by avoiding the slow start session and congestion control mechanism of TCP protocol. Usually the
 * throughput of RBUDP can overperform single stream TCP or even parallel TCP, especially over long, fat pipes.
 *
 * If you are writing a sender you need to instantiate a QUANTAnet_rbudpSender_c object. Likewise if you are writing a
 * Receiver youneed to instantiate a QUANTAnet_rbudpReceiver_c object.
 */
